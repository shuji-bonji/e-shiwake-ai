...
You can manage bookkeeping entirely through an AI agent — no UI needed. Copy-paste-ready prompt examples are provided.

**→ [Sample Prompts (English)](./docs/sample-prompts.en.md)**

Covers daily expenses, business-personal splits, revenue, withholding tax, financial reports, monthly routines, and more.
...
## E2E Testing (Cowork Skill)

An E2E test skill is included that verifies all MCP tools by actually calling them. It can be used with Claude Desktop's Cowork mode.
...
1. Drag and drop `skills/e-shiwake-e2e.skill` into Cowork
2. Or add the `skills/e-shiwake-e2e/` directory as a skill in Cowork settings

### Usage

With the e-shiwake MCP server connected, use trigger phrases like:

> "Run e2e tests"

> "Health check e-shiwake"
...
- Run specific phases: "Run only Phase 1"
- Test reports only: "Test only reports"
- Custom test year is supported (avoid years with real data)
